...
const AreaPhysiotherapyDetailPage = ({ citySlug, areaSlug, serviceSlug }) => {
  ...
  const [isLoading, setIsLoading] = useState(true);
  // Ref (not state) for the pause flag — hovering must NOT tear down and
  // recreate the scroll effect, it should just flip a value the already
  // running animation loop reads on the next frame.
  ...
  // ── Auto-scroll ──────────────────────────────────────────────────────────
  // Depends only on isLoading/safeAreas.length (i.e. whether the container
  // actually exists yet) — NOT on the paused flag, so hovering doesn't tear
  // down and recreate the loop. scrollPosition is seeded from the
  // container's actual current scrollLeft, so pausing (hover, or the
  // left/right arrow buttons) and resuming continues from that exact spot
  ...
  useEffect(() => {
    ...
    const smoothScroll = () => {
      if (!isPausedRef.current && container) {
        ...
        // Stay in sync in case scrollBy()/manual drag moved it while paused.
