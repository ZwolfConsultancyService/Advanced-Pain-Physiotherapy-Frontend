...
//   // ✅ FIX: Agar city nahi mili toh afterIn ko areaSlug maano
//   // Aur city ko state se ya default se lo
//   if (!citySlug) {
//     areaSlug = afterIn;
//     citySlug = null;
//   }
...
//   // ✅ FIX: citySlug state se bhi lo agar URL mein nahi hai
//   const citySlugResolved = useMemo(() => {
//     if (citySlug) return citySlug;
//     if (location.state?.citySlug) return location.state.citySlug;
//     return null;
//   }, [citySlug, location.state]);

//   const cityName = useMemo(
//     () => location.state?.cityName || getCityNameFromSlug(citySlugResolved || ""),
//     [citySlugResolved, location.state]
//   );

//   const currentCityData = useMemo(
//     () =>
//       safeCitiesData.find((city) => city.slug === citySlugResolved) ||
//       safeCitiesData[0] ||
//       { areas: [] },
//     [citySlugResolved]
//   );
...
//   // ✅ FIX: handleAreaClick mein citySlugResolved pass karo state mein
//   const handleAreaClick = (area) => {
//     const url = buildAreaUrl(citySlugResolved, area, serviceSlug);
//     navigate(url, {
//       state: {
//         cityName,
//         citySlug: citySlugResolved,
//         areaName: area,
//         serviceName,
//       },
//     });
...
//   if (!serviceSlug && citySlugResolved) {
//     return <CityOverviewInline citySlug={citySlugResolved} />;
//   }
...
//                 <span className="cursor-pointer hover:text-white hover:underline" onClick={() => navigate("/")}>Home</span>
//                 <span className="text-white/60">›</span>
//                 <span className="cursor-pointer hover:text-white hover:underline" onClick={() => navigate(buildCityUrl(citySlugResolved))}>{cityName}</span>
//                 <span className="text-white/60">›</span>
//                 <span className="text-white font-medium">{serviceName}</span>
//               </nav>
...
const AreaDetailPage = () => {
  ...
  const [isLoading, setIsLoading] = useState(true);
  const scrollContainerRef = useRef(null);
  // Ref (not state) for pause flag — hovering must NOT re-run the scroll effect,
  // it should just flip a flag the running animation loop reads each frame.
  ...
  // ── Auto-scroll ──────────────────────────────────────────────────────────
  // Only depends on areaSlug/serviceSlug (i.e. whether this section is shown
  // at all) — NOT on the paused flag, so hovering doesn't tear down and
  // recreate the loop. scrollPosition is seeded from the container's actual
  // current scrollLeft, so if the user dragged it manually the loop picks up
  // from there instead of snapping back to 0.
  useEffect(() => {
    // isLoading gates whether the slider's DOM node exists yet — the effect
    // must re-run once it flips to false, otherwise it fires while the
    // loading spinner is still showing, finds scrollContainerRef.current
    // null, and bails out forever.
    if (isLoading || areaSlug || !serviceSlug) return;
    const container = scrollContainerRef.current;
    if (!container) return;
    ...
    const scroll = () => {
      if (!isPausedRef.current && container) {
        ...
        // Stay in sync with the container in case the user manually
        // scrolled/dragged it while paused, so resume continues from there.
    ...
    animationFrameId = requestAnimationFrame(scroll);
    return () => { if (animationFrameId) cancelAnimationFrame(animationFrameId); };
  }, [isLoading, areaSlug, serviceSlug]);
